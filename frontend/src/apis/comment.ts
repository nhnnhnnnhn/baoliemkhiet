...
const commentApi = {
  ...
  getComments: async (
    ...
    page = 1,
    limit = 10
  ): Promise<Comment[] | GetCommentsResponse> => {
    try {
      // Thêm debug log
      console.log(
        `Fetching comments for article ${articleId}, page ${page}, limit ${limit}`
      );
  ...
  reportComment: async (params: CommentReportParams): Promise<any> => {
    try {
      console.log(
        `Reporting comment ${params.commentId} with reason: ${params.reason}`
      );
  ...
  getAllComments: async (
    ...
    try {
      console.log(`Fetching all comments, offset ${offset}, limit ${limit}`);
