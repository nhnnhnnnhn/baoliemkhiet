  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<GetCommentsResponse>) => {
      ...
      .addCase(
        ...
        (state, action: PayloadAction<Comment>) => {
          ...
          } else if (action.payload.status === "REJECTED_BY_AI") {
            state.createCommentError =
              "Bình luận của bạn đã bị từ chối vì có thể chứa nội dung không phù hợp";
            state.createCommentSuccess = false;
          }
        }
      )
        ...
        (state, action: PayloadAction<Comment>) => {
          ...
          if (action.payload.status === "APPROVED") {
            ...
            state.updateCommentError =
              "Bình luận của bạn đã bị từ chối vì có thể chứa nội dung không phù hợp";
            state.updateCommentSuccess = false;
            // Remove rejected comment from display
            state.comments = state.comments.filter(
              (comment) => comment.id !== action.payload.id
            );
            state.totalComments -= 1;
          } else {
            // If comment status changed to something other than APPROVED or REJECTED_BY_AI, remove it
            state.comments = state.comments.filter(
              (comment) => comment.id !== action.payload.id
            );
            state.totalComments -= 1;
      ...
      .addCase(handleUpdateComment.rejected, (state, action) => {
        state.isUpdatingComment = false;
        state.updateCommentError =
          (action.payload as string) || "Failed to update comment";
        state.updateCommentSuccess = false;
      })
      ...
      .addCase(handleDeleteComment.rejected, (state, action) => {
        ...
        state.deleteCommentError =
          (action.payload as string) || "Failed to delete comment";
        state.deleteCommentSuccess = false;
      });
  },
});
...
export const selectCreateCommentLoading = (state: RootState) =>
  state.comment.isCreatingComment;
export const selectCreateCommentError = (state: RootState) =>
  state.comment.createCommentError;
export const selectCreateCommentSuccess = (state: RootState) =>
  state.comment.createCommentSuccess;

// Update comment selectors
export const selectUpdateCommentLoading = (state: RootState) =>
  state.comment.isUpdatingComment;
export const selectUpdateCommentError = (state: RootState) =>
  state.comment.updateCommentError;
export const selectUpdateCommentSuccess = (state: RootState) =>
  state.comment.updateCommentSuccess;

// Delete comment selectors
export const selectDeleteCommentLoading = (state: RootState) =>
  state.comment.isDeletingComment;
export const selectDeleteCommentError = (state: RootState) =>
  state.comment.deleteCommentError;
export const selectDeleteCommentSuccess = (state: RootState) =>
  state.comment.deleteCommentSuccess;
