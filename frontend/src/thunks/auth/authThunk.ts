...
export const handleLogin = createAsyncThunk("auth/login", async (payload: LoginPayload, { rejectWithValue }) => {
  try {
...
export const handleGetProfile = createAsyncThunk("auth/getProfile", async (_, { rejectWithValue }) => {
  try {
...
export const handleSendOtp = createAsyncThunk("auth/sendOtp", async (payload: OtpPayload, { rejectWithValue }) => {
  try {
...
export const handleChangePassword = createAsyncThunk(
  ...
  async (payload: ChangePasswordPayload, { rejectWithValue }) => {
    try {
      const response = await authApi.changePassword(payload.email, payload.oldPassword, payload.newPassword)
      return response
    } catch (error: any) {
      if (error.response?.status === 400) {
...
export const handleGetUserById = createAsyncThunk("auth/getUserById", async (id: number, { rejectWithValue }) => {
  try {
...
export const handleDeleteUser = createAsyncThunk("auth/deleteUser", async (id: number, { rejectWithValue }) => {
  try {
