...
import {
  ...
  handleUpdateComment,
  handleDeleteComment,
} from "@/src/thunks/comment/commentThunk";
import {
  selectComments,
    ...
    content: [
      {
        type: "text",
        value:
          "Nội dung bài viết đang được tải, vui lòng đợi trong giây lát...",
      },
    ],
    stats: {
      views: 0,
...
export default function ArticlePage({
    ...
    if (typeof window !== "undefined") {
      ...
      if (savedReportedComments) {
        try {
          ...
          console.error(
            "Failed to parse reported comments from localStorage:",
            error
          );
        }
      }
      ...
      const isArticleReported =
        localStorage.getItem(`reported-article-${article.id}`) === "true";
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && reportedComments.length > 0) {
      localStorage.setItem(
        "reportedComments",
        JSON.stringify(reportedComments)
      );
    }
  }, [reportedComments]);
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        const articleId = Number(resolvedParams.slug);
        if (isNaN(articleId)) {
        ...
        console.log("Author details:", {
          ...
          authorProps: response.author
            ? Object.keys(response.author).join(", ")
            : "none",
          ...
          content:
            processedContent.length > 0
              ...
              : [
                  {
                    type: "text",
                    value: response.content || "Nội dung đang được cập nhật",
                  },
                ],
          ...
          articleData.relatedArticles = otherArticles.map((item: Article) => {
            ...
            if (item.publishedAt) {
              try {
                ...
                console.error(
                  "Lỗi khi định dạng thời gian bài viết liên quan:",
                  err
                );
              }
            ...
            return {
              ...
              thumbnail:
                item.thumbnail || "/placeholder.svg?height=200&width=300",
            };
          });
        } catch (error) {
    ...
    if (resolvedParams.slug && !isNaN(Number(resolvedParams.slug))) {
      dispatch(
        handleGetComments({ articleId: Number(resolvedParams.slug) }) as any
      );
    }
  ...
  useEffect(() => {
    ...
    if (createCommentError) {
      toast({
        title: "Bình luận không được chấp nhận",
        description: createCommentError,
        variant: "destructive",
      });
      dispatch(clearCreateCommentState());
    }
  ...
  useEffect(() => {
    ...
    if (updateCommentError) {
      toast({
        title: updateCommentError.includes("từ chối")
          ? "Bình luận không được chấp nhận"
          : "Lỗi cập nhật bình luận",
  ...
  const handleSaveComment = async () => {
    if (!editingCommentId || !editingCommentContent.trim() || !isAuthenticated)
      return;
  ...
  return (
    <>
        ...
        <div className="mb-8">
          ...
          <div className="flex justify-end gap-2 mb-6">
            ...
            >
              ...
              <span>{articleReported ? "Đã báo cáo" : "Báo cáo"}</span>
            </Button>
          </div>
        </div>
              ...
              >
                <Avatar className="h-16 w-16">
                  <AvatarImage
                    ...
                    alt={article.author.fullname || article.author.name}
                  />
                  <AvatarFallback>
                    {(article.author.fullname || article.author.name).charAt(0)}
                  </AvatarFallback>
                </Avatar>
              </Link>
            ) : (
      ...
      <AlertDialog
        open={articleReportDialogOpen}
        onOpenChange={setArticleReportDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
