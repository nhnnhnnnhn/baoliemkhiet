...
export default function ProfilePage() {
  ...
  const validateProfileForm = (): boolean => {
    ...
    if (!validateAddress(formValues.address)) {
    ...
    switch (name) {
      ...
      case 'bio':
        if (!validateBio(value)) {
          errorMessage = "Giới thiệu không được vượt quá 500 ký tự"
        }
        break
    }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      toast({
        title: "Lỗi",
        description: "Kích thước file không được vượt quá 2MB. Vui lòng chọn ảnh nhỏ hơn.",
        variant: "destructive",
        duration: 3000
      });
      return;
    ...
    try {
      ...
      if (response.success && response.file) {
        ...
        if (handleUpdateProfile.fulfilled.match(updateResult)) {
          ...
          if (handleGetProfile.fulfilled.match(profileResult)) {
            ...
            toast({
              title: "Lỗi",
              description: "Không thể cập nhật thông tin hồ sơ. Vui lòng chọn ảnh có kích thước nhỏ hơn 2MB và thử lại.",
              variant: "destructive",
              duration: 3000
            });
          }
          ...
          toast({
            title: "Lỗi",
            description: "Không thể cập nhật ảnh đại diện. Vui lòng chọn ảnh có kích thước nhỏ hơn 2MB và thử lại.",
            variant: "destructive",
            duration: 3000
          });
        }
        ...
        toast({
          title: "Lỗi",
          description: "Không thể tải lên ảnh. Vui lòng chọn ảnh có kích thước nhỏ hơn 2MB và thử lại.",
          variant: "destructive",
          duration: 3000
        });
      }
      ...
      if (error.response?.status === 413 || error.message?.includes('413')) {
        toast({
          title: "Lỗi",
          description: "Kích thước ảnh quá lớn, vui lòng chọn ảnh có kích thước nhỏ hơn 2MB",
          variant: "destructive",
          duration: 3000
        });
        return;
