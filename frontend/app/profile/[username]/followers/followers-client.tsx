...
export default function FollowersClient({ username }: FollowersClientProps) {
  ...
  // Fetch followers when profile user is available
  useEffect(() => {
          ...
          <div className="space-y-4">
            {filteredFollowers.map((follow) => (
              <div key={follow.id} className="bg-white rounded-lg shadow-sm p-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <Link href={`/profile/${follow.follower?.email?.split('@')[0]}`}>
                  ...
                  <div>
                    <Link href={`/profile/${follow.follower?.email?.split('@')[0]}`} className="font-medium hover:text-blue-600">
